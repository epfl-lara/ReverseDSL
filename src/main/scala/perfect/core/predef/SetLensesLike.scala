...
/**
  ...
  */
trait SetLensesLike { self: ProgramUpdater with ContExps with Lenses =>
  trait FiniteSetExtractor {
    def unapply(e: Exp): Option[(Seq[Exp], Seq[Exp] => Exp)]
  }

  trait FiniteSetAddExtractor {
    def unapply(e: Exp): Option[(Exp, Exp)]
    def apply(set: Exp, elem: Exp): Exp
  }

  class SetLensLike(FiniteSet: FiniteSetExtractor, FiniteSetAdd: FiniteSetAddExtractor) extends SemanticLens {
    def put(in: ContExp, out: ContExp)(implicit symbols: Symbols, cache: Cache): Stream[ContExp] = {
      in.exp match {
        ...
        case FiniteSetAdd(sExpr, elem) =>
          ...
          val (vs, builder) = FiniteSet.unapply(sExpr_v).getOrElse(return Stream.empty)
          val (vsNew, _) = FiniteSet.unapply(out.simplifiedExpr).getOrElse(return Stream.empty)
          val vsSet = vs.toSet
          ...
          changedElements match {
            case Some((old, fresh)) =>
              ...
              } else Stream.empty) #::: //Log.prefix("#2") :=
                (if (elem_v == old) {
