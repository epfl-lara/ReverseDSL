...
/**
  ...
  */
trait MapDataLensesLike { self: ProgramUpdater with ContExps with Lenses =>

  trait MapDataExtractor {
    def unapply(e: Exp): Option[(Seq[(Exp, Exp)], Exp, (Seq[(Exp, Exp)], Exp) => Exp)]
  }

  trait MapDataApplyExtractor {
    def unapply(e: Exp): Option[(Exp, Exp, String => Var)]
    def apply(e: Exp, g: Exp): Exp
  }

  class MapDataLensLike(MapData: MapDataExtractor, MapDataApply: MapDataApplyExtractor) extends SemanticLens {
