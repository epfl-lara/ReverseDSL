...
/**
  ...
  */
trait PatternMatchLenses { self: ProgramUpdater
  ...
  object PatternMatchLensGoal {
    def unapply(e: Exp) = extractPatternMatchGoal(e: Exp)
    def apply(pattern: Exp, vars: List[(Var, Exp)], forClone: Boolean) = buildPatternMatchGoal(pattern, vars, forClone)
  }
